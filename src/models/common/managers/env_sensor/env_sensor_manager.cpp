...
bool EnvSensorManager::init() {
  ...
  if (Serial) {
    if (aht20Available) Serial.println("[ENV] AHT20 (temp/humidite) OK");
    if (bmp280Available) Serial.println("[ENV] BMP280 (temp/pression) OK");
    if (!aht20Available && !bmp280Available)
      Serial.println("[ENV] Aucun capteur AHT20/BMP280 detecte");
  }
  return aht20Available || bmp280Available;
}
