...
// ============================================
void flushBuffer() {
  // Envoyer tout le buffer d'un coup
...
void render(const EyeConfig& left, const EyeConfig& right, float lookX, float lookY, float mouthState) {
  ...
  // Coordonnées dans le framebuffer
  int16_t lcx = LEFT_EYE_CX  + (int16_t)(lookX * LOOK_RANGE_X) - FB_X;
  int16_t rcx = RIGHT_EYE_CX + (int16_t)(lookX * LOOK_RANGE_X) - FB_X;
  int16_t ecy = EYE_CY + (int16_t)(lookY * LOOK_RANGE_Y) - FB_Y;
  int16_t mcx = MOUTH_CX - FB_X;
  int16_t mcy = MOUTH_CY - FB_Y;

  // Effacer le next buffer
  memset(s_fbNext, 0, FB_W * FB_H * sizeof(uint16_t));

  // Dessiner les 2 yeux + bouche dans le next buffer
  fbDrawShape(s_fbNext, lcx, ecy, left, COL_EYE);
  fbDrawShape(s_fbNext, rcx, ecy, right, COL_EYE);

  // Bouche
  if (mouthState < -0.15f) {
    // Bouche ouverte : contour cyan + intérieur sombre + langue rose
    float o = -mouthState;

    // Contour cyan (la forme extérieure)
    int16_t outerH = (int16_t)(16 + o * 22);
    int16_t outerW = (int16_t)(36 + o * 36);
    ...
    // Intérieur sombre (plus petit, décalé vers le bas)
    int16_t innerW = outerW - 12;
    int16_t innerH = outerH - 10;
    if (innerW < 6) innerW = 6;
    if (innerH < 4) innerH = 4;
    int16_t innerR = innerH / 2;
    fbFillRoundRect(s_fbNext, mcx, mcy + 2, innerW, innerH, innerR, COL_INNER);

    // Langue rose (en bas de l'intérieur)
    int16_t tongueW = innerW * 2 / 3;
    int16_t tongueH = innerH / 2;
    if (tongueH > 3) {
      fbFillRoundRect(s_fbNext, mcx, mcy + 2 + innerH/2 - tongueH/2,
                      tongueW, tongueH, tongueH/2, COL_TONGUE);
    }
  } else if (mouthState > 0.1f) {
    // Sourire : large et fin
    int16_t w = (int16_t)(30 + mouthState * 20);
    int16_t h = (int16_t)(6 + mouthState * 4);
    EyeConfig mCfg = { h, w, 0, 0, 3, 3, 0, 0 };
    fbDrawShape(s_fbNext, mcx, mcy, mCfg, COL_EYE);
  }
  // Zone -0.15 … 0.1 : pas de bouche (visage sans bouche par défaut)

  // Goutte de bave (pendant le sleep)
  float drool = BehaviorEngine::getStats().droolLength;
  float retract = BehaviorEngine::getStats().droolRetract;
  if (drool > 1.0f) {
    int16_t droolX = mcx + 6;
    int16_t droolTopY = mcy + 12;  // Bas de la bouche
    int16_t droolLen = (int16_t)drool;
    int16_t retractPx = (int16_t)retract;

    // Le fil commence à retractPx du haut (se rétracte vers la bouche)
    int16_t filStart = droolTopY + retractPx;
    int16_t filEnd = droolTopY + droolLen;

    // Fil de bave (trait fin) — seulement la partie visible
    if (filEnd > filStart) {
      int16_t filW = 3 - retractPx / 15; // Le fil s'amincit en se rétractant
      if (filW < 1) filW = 1;
      fbFillRect(s_fbNext, droolX - filW/2, filStart, filW, filEnd - filStart, COL_EYE);
    }

    // Goutte au bout (cercle)
