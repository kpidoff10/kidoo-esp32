...
bool SDManager::initSDCard() {
  ...
  // SD.begin() peut bloquer longtemps si pas de carte, on a un timeout global au niveau appelant
    ...
    delay(100);
    if (SD.begin(SD_CS_PIN, SPI, 250000)) return true;
    delay(100);
...
SDConfig SDManager::getConfig() {
  ...
  // Lire le theme Gotchi
  if (doc["gotchi_theme"].is<String>()) {
    String themeStr = doc["gotchi_theme"] | "boy";
    strncpy(config.gotchi_theme, themeStr.c_str(), sizeof(config.gotchi_theme) - 1);
