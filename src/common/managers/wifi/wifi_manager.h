...
public:
  ...
  /**
   * Connexion WiFi non bloquante (dans une tâche dédiée).
   * La tâche BLE reste réactive pendant la connexion.
   * @param ssid SSID du réseau
   * @param password Mot de passe du réseau
   * @param timeoutMs Timeout en millisecondes
   * @param callback Appelé quand la connexion est terminée (success = true/false)
   * @param userData Contexte passé au callback (doit survivre jusqu'au callback)
   */
  static void connectAsync(const char* ssid, const char* password, uint32_t timeoutMs,
...
private:
  ...
  // Connexion async (tâche dédiée)
  static void connectTaskFunction(void* parameter);
