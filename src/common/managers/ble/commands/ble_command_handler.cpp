...
void BLECommandHandler::sendSetupCompletionResponse(bool success, bool wifiConnected) {
  ...
  if (wifiConnected) {
    ...
    responseDoc["message"] = success ? "Configuration WiFi sauvegardee mais connexion echouee" : "Erreur lors de la configuration WiFi";
  }
  responseDoc["wifiConnected"] = wifiConnected;
  responseDoc["deviceId"] = uuid;
  #ifdef HAS_WIFI
  responseDoc["macAddress"] = macStr;
  #endif
  uint8_t brightnessPercent = (config.led_brightness * 100 + 127) / 255;
  ...
  pTxCharacteristic->notify();
  Serial.print("[BLE-COMMAND] Reponse envoyee (async): ");
...
bool BLECommandHandler::handleCommand(const String& data) {
  ...
  if (command == "setup") {
    ...
    if (BLESetupCommand::isValid(jsonData)) {
      ...
      // Réponse async en cours : le callback enverra la réponse
      if (BLESetupCommand::isAsyncPending()) {
        Serial.println("[BLE-COMMAND] Setup async en cours, reponse envoyee par callback");
        return false;
      }
      
      // Vérifier le statut WiFi après l'exécution (cas synchrone)
      #ifdef HAS_WIFI
